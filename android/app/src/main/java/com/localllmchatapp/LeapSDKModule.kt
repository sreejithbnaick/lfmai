...
class LeapSDKModule(reactContext: ReactApplicationContext) : NativeLeapSDKSpec(reactContext) {
    companion object {
        const val NAME: String = "NativeLeapSDK"
        const val TAG: String = "LeapSDKModule"
    }
    ...
    override fun createConversation(promise: Promise) {
        try {
    ...
    override fun generateResponse(input: String, promise: Promise) {
    ...
    override fun getHistory(promise: Promise) {
        try {
            val conversation = currentConversation
            if (conversation == null) {
                promise.resolve(createMap().apply {
